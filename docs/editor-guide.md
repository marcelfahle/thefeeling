...
- Oben der Tab **Visual**: die ganze Website in DatoCMS. Ein Klick auf eine Kachel, einen
  Text oder ein Bild öffnet den passenden Datensatz daneben.
- Nach dem Speichern aktualisiert sich die Vorschau automatisch (ca. 1–2 Sekunden).
- Bildschirmgrößen oben im Visual-Tab: Desktop 1440, Laptop 1280, Breakpoint 991 (ab hier
...
- **Layout** (nur Desktop, nur Collage): Kacheln direkt anordnen, siehe unten.
- **Share view-only link**: Link für Kund:innen/Kolleg:innen, 7 Tage gültig, nur ansehen.
...
- Pfeiltasten = 1 Schritt, mit Shift = 10 Schritte.
- Speed mit − / + im Etikett über der Kachel.
- ⌘Z rückgängig, ⇧⌘Z wiederholen.
- Die erste Kachel ist oben fixiert (nur seitlich verschiebbar).
- **Save as draft** speichert die Zahlen in DatoCMS als Entwurf. Danach wie gewohnt in
  DatoCMS **veröffentlichen**. Im Verlauf steht als Autor „Layout writer“.
...
- The **Visual** tab shows the whole site inside DatoCMS. Clicking a tile, text or image opens
  that record next to it.
- After saving, the preview updates by itself within a second or two.
- Viewport presets: Desktop 1440, Laptop 1280, Breakpoint 991 (mobile layout from here down),
  Phone 390.
...
- **Layout** (desktop collage only) lets you arrange tiles, see below.
- **Share view-only link** copies a link for clients or colleagues. It works for 7 days and is
  view only.
...
- Change speed with − / + on the label above the tile.
- ⌘Z undoes, ⇧⌘Z redoes.
- The first tile is pinned to the top (it can only move sideways).
- **Save as draft** writes the numbers to DatoCMS as a draft. Then **publish** in DatoCMS as usual.
